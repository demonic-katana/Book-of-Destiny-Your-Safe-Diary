...
from PyQt5.QtWidgets import QApplication, QMainWindow, QFileDialog, QInputDialog, QWidget, QDialog, QListWidget, \
    QListWidgetItem, QDialogButtonBox, QVBoxLayout, QLabel
...
class OwnWindow(QMainWindow):
    def __init__(self, db):
        ...
        # self.change_b_1.clicked.connect(self._change)
...
class ConfirmDel(QDialog):
    def __init__(self, item):
        super().__init__()
        self.setWindowTitle("Book of Destiny: Удаление")
        self.buttons = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        self.buttons.accepted.connect(self.accept)
        self.buttons.rejected.connect(self.reject)
        self.layout = QVBoxLayout()
        lable = QLabel("Вы действительно хотите безвозвратно удалить " + item + "?")
